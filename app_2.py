...
@st.cache_data
def build_world_map(filter_product="All"):
    """
    Clean world map — choropleth fill for approved countries,
    single scatter trace for all pins, labels in hover only.
    """
    import pycountry

    prod_full = {
        "LMS":"LiverMultiScan", "VS":"VitruvianScan",
        "Hepatica":"Hepatica",  "CoverScan":"CoverScan",
        "MRCP+":"MRCP+",        "LiverMultiScan":"LiverMultiScan",
    }

    markets = [
        {"country":"USA",         "iso3":"USA", "lat":38.5,  "lon":-96.0,  "flag":"🇺🇸", "authority":"FDA 510(k)",  "products":["LMS","VS","Hepatica","CoverScan","MRCP+"],  "cert":"K172685 – K241925"},
        {"country":"UK",          "iso3":"GBR", "lat":54.5,  "lon":-3.2,   "flag":"🇬🇧", "authority":"UKCA",        "products":["LMS","VS","Hepatica","CoverScan"],          "cert":"UKCA 752965"},
        {"country":"EU",          "iso3":"DEU", "lat":51.2,  "lon":10.0,   "flag":"🇪🇺", "authority":"CE / MDR",    "products":["LMS","VS","Hepatica","MRCP+"],              "cert":"MDR 735482 / CE 677486"},
        {"country":"Switzerland", "iso3":"CHE", "lat":46.8,  "lon":8.2,    "flag":"🇨🇭", "authority":"Swissmedic",  "products":["LMS","VS"],                                "cert":"CHRN-AR-20001035-MF-00093"},
        {"country":"UAE",         "iso3":"ARE", "lat":24.2,  "lon":54.5,   "flag":"🇦🇪", "authority":"MOHAP",       "products":["LMS","VS"],                                "cert":"DRCLAS-2026-002172 / 002152"},
        {"country":"Singapore",   "iso3":"SGP", "lat":1.35,  "lon":103.82, "flag":"🇸🇬", "authority":"HSA",         "products":["LMS","VS","Hepatica","CoverScan","MRCP+"], "cert":"Exempt from Registration"},
        {"country":"Malaysia",    "iso3":"MYS", "lat":4.5,   "lon":109.5,  "flag":"🇲🇾", "authority":"MDA",         "products":["LMS"],                                     "cert":"GB579821204818 / GB6824420-39123"},
    ]

    eu_iso3 = ["DEU","FRA","ITA","ESP","NLD","BEL","AUT","SWE","DNK",
               "FIN","NOR","IRL","PRT","GRC","POL","CZE","HUN","ROU"]

    # Apply product filter
    if filter_product != "All":
        markets = [m for m in markets
                   if filter_product in [prod_full.get(p, p) for p in m["products"]]]

    approved_iso3 = set()
    for m in markets:
        approved_iso3.add(m["iso3"])
        if m["iso3"] == "DEU":
            approved_iso3.update(eu_iso3)
    approved_iso3 = list(approved_iso3)

    # ── Choropleth ────────────────────────────────────────────────────────────
    all_iso3 = [c.alpha_3 for c in pycountry.countries]
    choropleth_df = pd.DataFrame({
        "iso3":     all_iso3,
        "approved": [1 if c in approved_iso3 else 0 for c in all_iso3],
    })

    fig = go.Figure()

    fig.add_trace(go.Choropleth(
        locations=choropleth_df["iso3"],
        z=choropleth_df["approved"],
        colorscale=[[0, "#D8E4ED"], [1, "#0D9488"]],
        showscale=False,
    ...
    # ── Single scatter trace for all pins ────────────────────────────────────
    lats, lons, texts, hovers, sizes = [], [], [], [], []

    for m in markets:
        prod_names = [prod_full.get(p, p) for p in m["products"]]
        count = len(prod_names)
        prod_lines = "  •  ".join(prod_names)

        lats.append(m["lat"])
        lons.append(m["lon"])
        texts.append(str(count))
        sizes.append(22 + count * 4)

        hovers.append(
            f"<b>{m['flag']}  {m['country']}</b><br>"
            f"<span style='color:#64748B'>{m['authority']}</span><br><br>"
            f"<b>Products cleared:</b>  {count}<br>"
            f"<span style='color:#475569; font-size:11px'>{prod_lines}</span><br><br>"
            f"<b>Cert / Reg:</b><br>"
            f"<span style='color:#475569; font-size:11px'>{m['cert']}</span>"
            f"<extra></extra>"
        )
    ...
    fig.update_layout(
        geo=dict(
            ...
            bgcolor="#F0F7FF",
            lataxis=dict(range=[-48, 72]),
            lonaxis=dict(range=[-165, 170]),
        ),
        paper_bgcolor="white",
    ...
    fig.add_shape(type="rect", xref="paper", yref="paper",
        x0=0.012, y0=0.102, x1=0.030, y1=0.118,
        fillcolor="#0D9488", line_color="white", line_width=1)
    fig.add_shape(type="rect", xref="paper", yref="paper",
        x0=0.012, y0=0.078, x1=0.030, y1=0.094,
        fillcolor="#D8E4ED", line_color="white", line_width=1)

    fig.add_annotation(
        x=0.008, y=0.065, xref="paper", yref="paper",
        text=(
            "  Approved market<br>"
            "  No current approval<br>"
            "<span style='color:#94A3B8'>  ● size = products cleared</span>"
        ),
        showarrow=False, align="left",
        bgcolor="rgba(255,255,255,0.92)",
        bordercolor="#CBD5E1", borderwidth=1, borderpad=8,
        font=dict(size=10, color="#0F1B33"),
    )
...
with tabs[0]:
    ...
    styled = matrix.set_index("Product").style.map(style_matrix)
    st.dataframe(styled, use_container_width=True, height=220)

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("### Global Regulatory Approvals Map")
    st.caption("Hover over a pin to see products cleared and certification numbers · Pin size reflects number of approved products")

    # Optional product filter for the map
    map_prod_filter = st.selectbox(
        "Highlight by product",
        ["All", "LiverMultiScan", "VitruvianScan", "Hepatica", "CoverScan", "MRCP+"],
        key="map_filter"
    )
    fig_map = build_world_map(map_prod_filter)
...
with tabs[1]:
    ...
    with c2:
        product_filter = st.selectbox("Filter by product",
            ["All"] + sorted(fda["Product"].unique().tolist()), key="fda_prod_filter")
...
with tabs[8]:
    ...
    with col1:
        mkt_filter = st.selectbox("Market", ["All"] + sorted(all_clearances["Market"].unique().tolist()), key="all_mkt_filter")
    with col2:
        prd_filter = st.selectbox("Product", ["All"] + sorted(all_clearances["Product"].unique().tolist()), key="all_prd_filter")
    with col3:
        search = st.text_input("Search cert number", placeholder="e.g. K213960", key="all_cert_search")
...
with tabs[9]:
    ...
    # ── World map ──
    st.markdown("#### 🌍 Global Regulatory Approvals Map")
    st.caption("Approved markets shown in teal · Hover pins for product and certification detail")
    fig_map_analytics = build_world_map("All")
    st.plotly_chart(fig_map_analytics, use_container_width=True, config={"displayModeBar": False})
